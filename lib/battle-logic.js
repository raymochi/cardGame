...
module.exports = function makeBattleLogic(dataHelpers) {
  ...
  return {
    ...
    emitToPlayers: function(match, io) {
      console.log('checking socket');
      if ( match.user1 && io.gameUserList[match.user1] ) {
        console.log('user and socket found, emitting' );
        io.gameUserList[match.user1].emit('gameState', this.formatMatchResponse(match, match.user1) );
      }
      if ( match.user2 && io.gameUserList[match.user2] ) {
        io.gameUserList[match.user2].emit('gameState', this.formatMatchResponse(match, match.user2) );
      }
