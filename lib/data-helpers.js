...
module.exports = function makeDataHelpers(db) {
  ...
  return {
    ...
    getUserDeck: function(userid) {
      return db
        .select('cards.id as id', 'decks.user_id as ownerId', 'cards.name as name', 'cards.type as cardType', 'creatures.front_power as frontPower', 'creatures.side_power as sidePower', 'creatures.type as creatureType', 'creatures.value as value' )
        .from('cards')
        .join('deck_cards', 'deck_cards.card_id', 'cards.id')
        .join('decks', 'deck_cards.deck_id', 'decks.id')
        .leftJoin('creatures', 'creatures.card_id', 'cards.id')
        .where('decks.user_id', userid) // TODO should be userid
        .then((rows) => {
          return rows;
        });
    },
    ...
    insertUser: function(username, password, email) {
      return db
        .insert({username: username, password: password, email: email, avatar: 'https://image.flaticon.com/icons/png/128/126/126486.png', rating: 1500, rating_deviation: 200, volatility: 0.06})
        .into('users');
    },
    ...
    insertCardToDeck: function(deckId, cardId, slot) {
      return db
        .insert({slot: slot, deck_id: deckId, card_id: cardId })
        .into('deck_cards');
    },

    setRating: function(userId, rat, dev, vol) {
